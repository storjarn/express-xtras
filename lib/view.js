module.exports = function(app) {
    ...
    // Handlebars for isomorphic templates
    if (config.handlebars && config.handlebars.server) {
        var Handlebars = require('handlebars');
        var Helpers = require('handlebars-helpers');
        Helpers.register(Handlebars);
        if (config.handlebars.helpers) {
            require(config.handlebars.helpers).register(Handlebars);
        }
        app.set('handlebars', require(config.handlebars.server)(Handlebars));
        app.locals.templates = app.get('handlebars');

        if (config.handlebars.lib) {
            config.scripts.push({
                src: config.handlebars.lib
            });

            if (config.handlebars.client) {
                config.scripts.push({
                    src: config.handlebars.client
                });
            }
        }
    }
